...
def get_parsed_msg(useraccount, password, limits=1, debuglevel=1):
    ...
    for item in mails[-1:-(limits+1):-1]:
        ...
        print(content_charset) # 此处传入了None，给charset
...
def parse_raw_mail_data(raw_lines, charset='utf8'):
    print('raw_lines_nums', len(raw_lines), type(raw_lines))
    print('raw_lines',raw_lines)
    msg_content = b'\r\n'
    for line in raw_lines:
        if line is not None:
            msg_content += line.decode(encoding=charset)
            print(msg_content)
...
if __name__ == '__main__':
    ...
    useraccount = 'jzq1131233593@163.com'
    password = 'wobuaiwoshi'
    limits = 10
    debug_level = 1
    # 获取到limits限制下的所有的邮件
